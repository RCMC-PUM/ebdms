...
class Order(Model):
    ...
    person_responsible = models.CharField(
        max_length=255, help_text="Person responsible for this particular order."
    )
    ...
    description = models.TextField(
        null=True, blank=True, help_text="Order description."
    )
...
class StockItem(Model):
    ...
    catalog_number = models.CharField(
        max_length=128, blank=True, null=True, help_text="Product catalog number."
    )
    ...
    expiration_date = models.DateField(
        blank=True, null=True, help_text="Product's expiration date."
    )
    ...
    def clean(self):
        ...
        if (
            self.available
            and self.item_type == self.ItemType.CHEMISTRY
            and not self.expiration_date
        ):
            raise ValidationError(
                {
                    "expiration_date": f"If the chemistry is available please specify expiration date!"
                }
            )
